...
public class DoctorController {
	...
	public String index(HttpServletRequest request, HttpSession session){
		...
		return "staff/doctor/doctorList";
	}
	
	//获取医生信息列表
	@RequestMapping("getDoctorList.do")
	@ResponseBody
	public PagedList getDoctorList(PagedList pagedList, UserEntity user,String hosType){
	...
	public String toPatientWin(HttpServletRequest request, String userSysId, HttpSession session){
		...
		return "staff/doctor/patientList";
	}
	...
	public PagedList getPatientList(PagedList pagedList, String userSysId, Patient patient){
		...
		return staffService.getPatientList(pagedList, userSysId, "8", patient);
	}
	
	//跳转0-新增,1-编辑护士信息页面
	@RequestMapping("toEditDoctorWin.do")
	public String toEditDoctorWin(HttpServletRequest request, String userId, Integer editFlag){
		request.setAttribute("hosType", "8");
		List<Dept> deptList = patientService.getDeptList();
		request.setAttribute("deptList", deptList);
		if(editFlag==1){
			UserEntity user = staffService.getUserByUserSysId(userId);
			request.setAttribute("user", user);
			return "staff/doctor/editDoctor";
		}else
			return "staff/doctor/addDoctor";
	}
	
	@RequestMapping("addDoctor.do")
	@ResponseBody
	public Object addDoctor(HttpServletRequest request,UserEntity user){
		user.setUserType("1");
	...
	@RequestMapping("delDoctor.do")
	@ResponseBody
	public Object delDoctor(String userId){
		UserEntity user = staffService.getUserByUserSysId(userId);
		if(user==null){
			return new ResultObject(false, "找不到相应数据，请刷新重试！");
		}
